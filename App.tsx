...
export default function App() {
  ...
  const [settings, setSettings] = useState<Settings>(() => {
    ...
    return s ? JSON.parse(s) : { pin: '', monthlyTarget: '1000000', stockAlert: '5000', exchangeRate: '3500', darkMode: false };
  });
...
function DepositsList({ records }: { records: RecordData[] }) {
  ...
  const deposits = monthRecords.flatMap(r => {
    const d = [];
...
function ExpensesList({ records }: { records: RecordData[] }) {
  ...
  const expenses = monthRecords.flatMap(r => 
